...
description:
  Build Takeoff React Spar forms with React Hook Form, schema validation, and
  consistent error display.
sidebar_label: React Hook Form
slug: /forms/react-hook-form
---
...
export const demoCode = String.raw`
// A picker is composed and controlled, so it lives in its own component and
// binds through Controller. \`field.onChange\` goes to the hook, not to
// Input.Field: that is the one channel both halves report through, and it
// carries a Date.
function DepartureField({ field, fieldState }) {
  ...
  return (
    <Field invalid={fieldState.invalid} required>
      ...
      <Popover {...picker.popoverProps}>
        <Input>
          {/* onBlur and name are the form's — inputProps carries neither. */}
          <Input.Field
            {...picker.inputProps}
            name={field.name}
            onBlur={field.onBlur}
      ...
      {fieldState.invalid ? <Field.ErrorMessage>{fieldState.error?.message}</Field.ErrorMessage> : <Field.Description>Pick a date or type it.</Field.Description>}
    </Field>
  );
}
...
function BookingRequestDemo() {
  const formSchema = z.object({
    ...
    departure: z.date({ message: 'Select a departure date.' }),
  ...
  const form = useForm({
    ...
    // A half-typed date is undefined until it is whole, so onTouched suits a
    // picker better than validating on every keystroke.
    mode: 'onTouched',
    defaultValues: {
      title: '',
      departure: undefined,
  ...
  return (
    <form id="form-rhf-demo" onSubmit={form.handleSubmit(() => {})} className="grid w-full max-w-90 gap-3">
      <Controller
        name="title"
      ...
      <Controller name="departure" control={form.control} render={DepartureField} />
      ...
      <Controller
        ...
        render={({ field, fieldState }) => (
          <Field invalid={fieldState.invalid} required>
            ...
            >
              <Select.Trigger placeholder="Select cabin" aria-invalid={fieldState.invalid} />
              <Select.Content>
            ...
            {fieldState.invalid ? <Field.ErrorMessage>{fieldState.error?.message}</Field.ErrorMessage> : null}
          </Field>
        )}
      />
        ...
        render={({ field, fieldState }) => (
          <Field invalid={fieldState.invalid} required>
            <div className="flex items-start gap-3">
              <Checkbox checked={field.value} onChange={field.onChange} name={field.name} aria-invalid={fieldState.invalid}>
                <Checkbox.Indicator />
              </Checkbox>
...
export const arrayFieldsDemoCode = String.raw`
function PassengerArrayDemo() {
  ...
  return (
    <form onSubmit={form.handleSubmit(() => {})} className="grid w-full max-w-90 gap-3">
...
A date picker is composed and controlled, so it takes the same `Controller`
route — never `register()`, which would bind the DOM input and collect the
_masked text_, while a day picked in the grid dispatches no DOM change event at
all. Give `field.onChange` to the **hook** rather than to `Input.Field`: that is
the one channel both halves report through, and it carries a `Date`.
...
<Controller name="departure" control={form.control} render={DateField} />;
```

Three things `inputProps` does not carry, because they belong to the form:
`onBlur` (without `field.onBlur` the `onTouched` / `onBlur` validation modes
never fire), `name`, and a settled value — the hook keeps the text but holds the
value at `undefined` until the date is whole, so `mode: 'onTouched'` suits it
better than `'onChange'`. Keep the field in its own component rather than an
inline `render` function: a hook inside `render={...}` works, but lint rules
read it as a conditional hook.
