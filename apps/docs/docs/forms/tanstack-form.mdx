...
title: TanStack Form
description:
  Build Takeoff React Spar forms with TanStack Form field state, validators, and
  consistent error display.
sidebar_label: TanStack Form
slug: /forms/tanstack-form
---
...
// binds through the field render function. \`field.handleChange\` goes to the
// hook, not to Input.Field: that is the one channel both halves report through,
// and it carries a Date.
function DepartureField({ field }) {
  ...
  return (
    <Field invalid={invalid} required>
      ...
      <Popover {...picker.popoverProps}>
        <Input>
          {/* handleBlur and name are the form's — inputProps carries neither. */}
          <Input.Field
            {...picker.inputProps}
            name={field.name}
      ...
      {invalid ? <Field.ErrorMessage>{message}</Field.ErrorMessage> : <Field.Description>Pick a date or type it.</Field.Description>}
    </Field>
  );
}
  ...
  return (
    ...
    >
      ...
      <form.Field name="departure">
        {(field) => <DepartureField field={field} />}
      </form.Field>
...
function rather than a DOM input. Give `field.handleChange` to the **hook**, not
to `Input.Field`: that is the one channel both halves report through, and it
carries a `Date`. `field.handleBlur` has to be added by hand — `inputProps` does
not carry it — and a half-typed date holds the value at `undefined` until it is
whole.
