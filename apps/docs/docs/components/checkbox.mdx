...
(`size`, plus inherited `invalid` / `disabled` / `required` / `readOnly`), and
`Checkbox.Indicator` is the bordered box that hosts the icon. Wrap `Checkbox` in
a `Field` to attach a form-level label, description, or error message — the
