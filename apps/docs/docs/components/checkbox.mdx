...
— `onChange` is always called with a plain `boolean`. In uncontrolled mode,
clear `indeterminate` from your own state in response to `onChange`; the
checkbox then shows the toggled value from that single interaction.
...
| <ApiBadge label="slotProps" />     | <code>Partial&lt;Record&lt;<a href="#checkbox-td-checkboxslot">CheckboxSlot</a>, React.HTMLAttributes&lt;HTMLElement&gt;&gt;&gt;</code> | -       | Per-slot HTML attribute overrides.                                                                                  |
