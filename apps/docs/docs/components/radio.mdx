...
| <ApiBadge label="invalid" />    | <code>boolean</code>                                                                | false     | Marks the group as visually invalid. Cascades to descendant `Radio.Item`s. When nested inside a `<Field>` with `invalid`, this is inherited automatically; a direct prop on `<Radio>` always wins. Maps to Spar's `isInvalid` for ARIA wiring (`aria-invalid`). |
