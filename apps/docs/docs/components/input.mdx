...
(`caret`, `previousValue`, `inputType`) and returns `{ value }` — optionally
with `raw`, `completed`, `iso` and its own `insignificant` separator pattern.
Caret placement stays inside Spar, so a resolver only decides what the value
_is_.
