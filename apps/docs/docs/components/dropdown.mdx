...
`Dropdown.Viewport` is the panel's scroll container. Wrap the items in it
whenever the list can grow long enough to need scrolling — it bounds the height
and keeps the highlighted item in view during keyboard navigation. The content
panel itself does not scroll, so a long menu without a viewport will overflow;
omit it only for short menus that always fit.
...
and typeahead keep the highlighted item in view. The native scrollbar is hidden
(as in Radix Select) — scroll with the wheel, trackpad, or keyboard.
