...
| <ApiBadge label="size" />       | <code><a href="#breadcrumb-td-breadcrumbsize">BreadcrumbSize</a></code>                   | 'base'  | Density scale. Emitted as `data-size` on the root `<nav>`; the recipe scopes every part from there. Shared with every part through context. |
| <ApiBadge label="type" />       | <code><a href="#breadcrumb-td-breadcrumbtype">BreadcrumbType</a></code>                   | 'basic' | Visual style. Emitted as `data-type` on the root `<nav>`; the recipe scopes every part from there. Shared with every part through context.  |
...
| ----------------------------------------------- | ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <ApiBadge label="data-slot=&quot;root&quot;" /> | Always            | Stable selector for wrapper styling on the root slot.                                                                                                                         |
| <ApiBadge label="data-position" />              | Always            | `"first"`, `"middle"`, or `"last"` in list order. Emitted by Spar: items register with `Breadcrumb.List` through context, so positions recompute as crumbs mount and unmount. |
| <ApiBadge label="data-current" />               | On the last item. | Marks the current crumb; emitted by Spar on the last registered item, alongside `Breadcrumb.Page`’s own `data-current`.                                                       |
