...
| Public API                | `<Input.Field mask={…} onValueChange={(value, meta) => …} />`. Four forms, in order of reach: a `blocks` shape, the `date` / `time` / `number` presets, an L3 `regex`, or a resolver function. `meta` carries `raw`, `completed` and `iso`. The preset factories (`createDateMask`, `createTimeMask`, `createNumberMask`) are re-exported so a built-in can be wrapped rather than reimplemented.                   |
...
| Explicit non-goals (v1)   | No wrapper-owned formatters, no `disableMask` flag (a mask is present or absent), no masked `Input.Chips`, no date-picker composition inside `Input` (that is a pattern — see `datepicker-contract.md`), and no `data-*` beyond the `data-mask` / `data-mask-completed` Spar already emits.                                                                                                                         |
