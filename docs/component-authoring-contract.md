...
| Explicit non-goals (v1)   | No object-array values, no `chipLabelKey` / `chipOptions` / `chipDisabled` (tk-select-coupled), no paste-splitting, no `aria-live` announcements, no masking **of the chip entry field** (field-level `mask` is its own capability — see the Input mask contract; combining the two is untested), no adapter hook, and no speculative state `data-*`. |
...
This contract covers `Input.Field`'s `mask` (Core `tk-datepicker`'s
`disableMask` and every hand-rolled `onChange` formatter it replaces).

| Decision area             | Contract                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Behavior owner            | **Spar owns all of it** — the mask engine, controlled/uncontrolled reconciliation, and caret placement, which is the part this exists for. Spar reads `beforeinput`, where the operation is stated outright, rather than inferring it by diffing old and new value. takeoff-spar re-exports the vocabulary and picks two props; it implements no formatting.                                                        |
| Public API                | `<Input.Field mask={…} onValueChange={(value, meta) => …} />`. Four forms, in order of reach: a `blocks` shape, the `date` / `time` / `number` presets, a `regex`, or a resolver function. `meta` carries `raw`, `completed` and `iso`. The preset factories (`createDateMask`, `createTimeMask`, `createNumberMask`) are re-exported so a built-in can be wrapped rather than reimplemented.                       |
| Public parts              | **None.** A mask is a prop on an existing part, not anatomy — it adds no node, no slot and no `data-*`. A masked field is the same `Input.Field` every other example uses.                                                                                                                                                                                                                                          |
| Internal decorative slots | None.                                                                                                                                                                                                                                                                                                                                                                                                               |
| Upstream Spar changes     | The engine itself (turkishtechnology/spar#190) — this wrapper cannot ship until it does. Nothing further: `meta.iso` and `meta.completed` already give a parse-free value, so no wrapper-side normalization is needed.                                                                                                                                                                                              |
| The one wrapper-owned bit | `InputField` refreshes the Input context mirror from `onValueChange`. Spar applies deletes and undo/redo imperatively — it writes `element.value` and preventDefaults the event — so those edits never reach `onInput` / `onChange`, and without the mirror `Input.ClearButton` would linger after a backspace-to-empty and `Input.Strength` would freeze. This is state coordination, not re-implemented behavior. |
| Explicit non-goals (v1)   | No wrapper-owned formatters, no `disableMask` flag (a mask is present or absent), no masked `Input.Chips`, no date-picker composition inside `Input` (that is a pattern — see `docs/datepicker-contract.md`), and no `data-*` beyond the `data-mask` / `data-mask-completed` Spar already emits.                                                                                                                    |
| Implementation order      | 1. Land the engine in Spar with its own tests. 2. `Pick` `mask` / `onValueChange` with an intent comment. 3. Re-export the `Mask*` vocabulary from the Input barrel rather than re-declaring it. 4. Mirror `onValueChange` into the Input context. 5. Tests, docs/API tables, changeset.                                                                                                                            |
| Blockers                  | If a caret or delete behaves wrongly, fix Spar — do not correct it in the wrapper. A formatter wired into `onChange` is the failure mode this replaces: it never sees the imperative edits, and it derives the caret by diffing, which is where caret bugs live.                                                                                                                                                    |
