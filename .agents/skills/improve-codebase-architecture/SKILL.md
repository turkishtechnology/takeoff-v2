...
- **Naming a deepened module after a concept not in `CONTEXT.md`?** Add the term
  to `CONTEXT.md` — same discipline as `/grill-with-docs` and its
  `CONTEXT-FORMAT` spec. Create the file lazily if it doesn't exist.
...
- **User rejects the candidate with a load-bearing reason?** Offer an ADR,
  ...
  reasons ("not worth it right now") and self-evident ones. Use the team's
  ADR-FORMAT convention if one exists, otherwise a brief context + decision +
  consequences markdown stub.
