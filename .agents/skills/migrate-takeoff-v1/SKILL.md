...
## Default mode: agent-led migration

Use an AI coding assistant as the primary migration operator. Do not ask teams
to manually repeat the inventory, mapping, prop translation, and verification
for every screen. The assistant should read the scanner output, group work by
screen, migrate one bounded screen per change, run the focused checks, and
present the diff and remaining gaps for human review. A developer still owns the
product decision for visual behavior, gap handling, and merge approval.

## Workflow

### 1. Inventory

From this skill directory, run the scanner against the consumer source or app:

```bash
node scripts/scan-v1-usage.mjs <app-dir>
node scripts/scan-v1-usage.mjs <app-dir> --json > v1-usage.json
```

Read the report before editing. Use its component counts, attributes, handlers,
CSS findings, React version, and blockers to size the migration.
...
### 3. Let the assistant migrate leaf-first

Give the assistant the scanner report and the path to the consumer app. For each
screen, it should:
...
3. For a direct or compound target, open the target `takeoff-<name>` skill in
   the v2 repository and use its API as the source of truth.
...
- rerun the scanner and confirm only intentional gap allowlist usage remains;
...
- remove v1 packages only after the scanner reports zero remaining usage outside
  the documented gap allowlist.
